...
struct bf_contact_info {
  char *number;

  struct bf_contact_info *next;
};
...
struct bf_context_values_struct {
  ...
  int wifi_enabled;
  char wifi_ssid[33];
  char gps[3]; // there are a lot of problems reducing doubles to 3 bytes... but
               // it's what we're doing for now
  struct app_context_entry
      *app_context_queue; // list of apps to check running state of
};
